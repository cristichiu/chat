        ...
        if(!token) {
            ...
            if(!grupId) {
                ...
                switch(option) {
                    ...
                    case 5: {
                        printf("grup_public_id: "); scanf("%ld", &grupId);
                        break;
                    }
                    case 6: {
                        token = 0;
                ...
                int option;
                printf("1: Adauga un utilizator la un grup\n2: Vezi toti utilizatorii din grup\n3: Scrie un mesaj\n4: Vezi toate mesajele\n5: accepta invitatia intr-un grup\n6: kick\n7: Go Home\n");
                scanf("%d", &option);
                Grups grup = getGrupByLId(grupId, GS_FOR_PUBLIC_ID);
                if(!grup.id) {
                    option = 100;
                    grupId = 0;
                }
                switch(option) {
                    case 1: {
                        ...
                        if(addMemberInGrup(grupId, user.id, user_public_id, permissions)) printf("Ceva nu a mers cum trebuie\n"); else printf("User adaugat cu succes\n");
                        break;
                    }
                    case 2: {
                        Grups grup = getGrupByLId(grupId, GS_FOR_PUBLIC_ID);
